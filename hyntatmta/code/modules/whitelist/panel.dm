...
/datum/admins/proc/bwhitelist_panel(var/ckeyname = null)
	...
	output += "<a href='?src=[UID()];whitelistpanel=tab;tab=0' [wl_tab == 0 ? "class='linkOn'" : ""]>Whitelist</a>"
	output += "<a href='?src=[UID()];whitelistpanel=tab;tab=1' [wl_tab == 1 ? "class='linkOn'" : ""]>Applications</a>"
	output += "</center>"
	output += "<HR>"

	switch(wl_tab)
		if(0)	//Whitelist Panel
		...
		if(1)	//Application's Panel
			output += "kek" //WIP

	var/datum/browser/popup = new(usr, "whitelistpanel", "<div align='center'>Whitelist Panel</div>", 600, 600)
